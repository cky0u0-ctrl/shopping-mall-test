...
export default function Cart() {
    ...
    // 체크된 항목을 저장할 변수 구매하기에 념겨줄 변수값
    const [checkedItems, setCheckedItems] = useState([]);
    // 체크박스를 체크했을 때 실행할 메서드
    const handleChecked = (item) => {
        console.log(item);
        const key = `${item.id}-${item.size}`;
        setCheckedItems((prev) => prev.includes(key) ? prev.filter((v) => v !== key) : [...prev, key])
    }
    console.log("cart내용", cartItems)
    console.log("선택된 항목을 표시할 key:", checkedItems);


    // 전체 체크 메서드
    const handleAllChecked = (e) => {
        // 체크가 되면: 모든 요소를 checkedItems에 넣기
        if (e.target.checked) {
            const allKeys = cartItems.map((item) => `${item.id}-${item.size}`);
            setCheckedItems(allKeys);
        } else {
            // 체크 해제 시: checkedItems 비우기
            setCheckedItems([]);
        }
    };
    ...
    const selectedTotal = selectedItems.reduce((acc, cur) => acc + cur.price * cur.count, 0);
            ...
            <div className="inner">
                ...
                <div className="cart-wrap">
                    ...
                    <div className="cart-bottom">
                        ...
                        {/* <Link to="/payment" sendData={ selectedItems } sendTotal={ selectedTotal } /></div>모두 구매하기</Link> */}
